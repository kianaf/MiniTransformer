...
struct AdamOptimizer {
    ...
    AdamOptimizer(ModelArch& arch, double alpha=0.001, double beta1=0.9, double beta2=0.999): 
        arch(arch), alpha(alpha), beta1(beta1), beta2(beta2), m(ModelParams(arch,PINIT_ZERO)), v(ModelParams(arch,PINIT_ZERO)) {
    }

    void update(ModelParams& params, ModelParams& d_params) {
        t++;

        for (int i = 0; i < arch.nweights(); i++) {
            m.W[i] = beta1 * m.W[i] + (1.0 - beta1) * d_params.W[i];
            v.W[i] = beta2 * v.W[i] + (1.0 - beta2) * d_params.W[i]*d_params.W[i];

            double m_hat = m.W[i] / (1 - pow(beta1,t));
            double v_hat = v.W[i] / (1 - pow(beta2,t));

            params.W[i] = params.W[i] - alpha * m_hat / (sqrt(v_hat) + eps);
        }

        for (int i = 0; i < arch.nbiases(); i++) {
            m.b[i] = beta1 * m.b[i] + (1.0 - beta1) * d_params.b[i];
            v.b[i] = beta2 * v.b[i] + (1.0 - beta2) * d_params.b[i]*d_params.b[i];

            double m_hat = m.b[i] / (1 - pow(beta1,t));
            double v_hat = v.b[i] / (1 - pow(beta2,t));

            params.b[i] = params.b[i] - alpha * m_hat / (sqrt(v_hat) + eps);
        }
    }
};
...
double lossFun_grad(ModelParams* params, ModelParams* d_params, double* input, int n, int p, 
...
void generate_data(std::vector<double> &input, std::vector<int> &startvec, 
                    int n, int p, int pos1, int pos2, int pos3, int maxlen) {
...
void permute_meansq(std::vector<double>& tsq, int ncont, int ntar, int curpos, double curcum, std::vector<double>& collection) {
    ...
    for (int i = 0; i < ncont; i++) {
        ...
        if (curpos == ntar-1) {
            collection.push_back(curval*curval/(ntar*ntar));
        } else {
        ...
        std::vector<std::vector<double>>& context, std::vector<std::vector<double>>& target, int predindex) 
{
    ...
    std::vector<double> meansq(ncont,0.0); // this saves the square of the mean of differences
    std::vector<double> tsq(ncont*ntar);  // only saves differences
...
int main() {
    ...
    std::vector<int> index(startvec.size());
    std::iota(index.begin(), index.end(), 0);

    AdamOptimizer adam(arch,alpha);
    ...
    for (int i = 0; i < nepochs; i++)
    {
        ...
        for (int& j : index) {
            ...
            sumloss += lossFun_grad(&params, &d_params, curinput, curn, p, &scratch, &d_scratch);

            //params.update(d_params, nu);
            adam.update(params, d_params);
            d_params.zero();
        }
        //std::cout << "ave loss " << sumloss/startvec.size() << std::endl;
    }
    ...
    std::vector<double> avepval(context.size(),0.0);

    int nrepp = 10;
    for (int i = 0; i < nrepp; i++) {
        std::vector<std::vector<double>> targetall, target;
        ...
        for (int j = 0; j < context.size(); j++) avepval[j] += pval[j];
    }

    for (int i = 0; i < context.size(); i++) avepval[i] /= nrepp;
    
    std::cout << "Pvals: " << std::endl;
    for (int i = 0; i < context.size(); i++) {
        std::cout << i+1 << ": " << avepval[i] << std::endl;
    }   
