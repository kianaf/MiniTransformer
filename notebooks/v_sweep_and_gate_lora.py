"""V-sweep and predictability-gate analysis on LORA cohort data.

Mirrors notebooks/v_sweep_and_gate.py (simulation version) but applied to one
...
(B) On the same fitted model, runs the permutation test for V in {5, 6, 7}
    ...
    per-variable mean p-value, std, KS distance, and rejection rates at
...
Outputs (under notebooks/results/v_sweep_and_gate_lora/<data_str>/):
    ...
    v_sweep_plots.png        mean p-value, rejection rate, KS distance vs V
...
OUT_DIR = f"notebooks/results/v_sweep_and_gate_lora/{DATA_STR}"
...
def main():
    ...
    # 7. Plot: 3 panels (mean p, rej rate at 0.05, KS distance) x V
